...
#define maxdD 9                         // LUT is used for d <= D, D <= 9
...
#define flutes_ALLD(d, xs, ys, s, acc) \
    (d<=maxdD ? flutes_LD(d, xs, ys, s) \
          : (d<=D1(acc) ? flutes_MD(d, xs, ys, s, acc) \
                        : flutes_HD(d, xs, ys, s, acc)))

#define flutes_wl_LMD(d, xs, ys, s, acc) \
    (d<=maxdD ? flutes_wl_LD(d, xs, ys, s) : flutes_wl_MD(d, xs, ys, s, acc))
#define flutes_LMD(d, xs, ys, s, acc) \
    (d<=maxdD ? flutes_LD(d, xs, ys, s) : flutes_MD(d, xs, ys, s, acc))

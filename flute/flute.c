...
struct csoln
{
    ...
    unsigned char rowcol[maxdD-2];  // row = rowcol[]/16, col = rowcol[]%16,
    unsigned char neighbor[2*maxdD-2];
};
struct csoln *LUT[maxdD+1][MGROUP];  // storing 4 .. D
int numsoln[maxdD+1][MGROUP];
...
DTYPE flutes_wl_LD(int d, DTYPE xs[], DTYPE ys[], int s[])
{
    int k, pi, i, j;
    struct csoln *rlist;
    DTYPE dd[2*maxdD-2];  // 0..D-2 for v, D-1..2*D-3 for h
...
void local_refinement(Tree *tp, int p)
{
    ...
    DTYPE x[MAXD], xs[maxdD], ys[maxdD];
    int ss[maxdD];
    Tree tt;

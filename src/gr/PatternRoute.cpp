...
std::string SteinerTreeNode::getPythonStringfixed(std::shared_ptr<SteinerTreeNode> node) {
    vector<std::pair<utils::PointT<int>, utils::PointT<int>>> edges;
    preorder(node, [&] (std::shared_ptr<SteinerTreeNode> node) {
        for (auto child : node->children) {
            edges.emplace_back(*node, *child);
...
std::string PatternRoutingNode::getPythonStringfixed(std::shared_ptr<PatternRoutingNode> routingDag) {
    vector<std::pair<utils::PointT<int>, utils::PointT<int>>> edges;
    std::function<void(std::shared_ptr<PatternRoutingNode>)> getEdges = 
        [&] (std::shared_ptr<PatternRoutingNode> node) {
            for (auto& childPaths : node->paths) {
...
void PatternRoute::constructSteinerTree() {
    ...
    gridGraph.selectAccessPoints(net, selectedAccessPoints);//选择相通的点？
    ...
    if (degree == 1) {
        ...
        Tree flutetree = flute(degree, xs.get(), ys.get(), ACCURACY);//调用flute
        const int numBranches = degree + degree - 2;
        //std::cout<<"numBranches: "<<numBranches<<std::endl;
...
// 构建路由DAG
/*这段C++代码定义了一个名为`PatternRoute`的类中的`constructRoutingDAG`方法，该方法用于构造路由DAG（有向图）。实现原理是使用递归的方式来遍历一个Steiner树，并将其节点转换为PatternRoutingNode对象。最后，将构造的PatternRoutingNode对象添加到路由DAG中。

代码解释：

1. `void PatternRoute::constructRoutingDAG()`：定义在`PatternRoute`类中的一个方法，用于构造路由DAG。
2. `std::function<void(std::shared_ptr<PatternRoutingNode>&, std::shared_ptr<SteinerTreeNode>&)> constructDag`：定义一个函数对象，该对象接受两个参数：一个指向PatternRoutingNode的shared_ptr和一个指向SteinerTreeNode的shared_ptr。
3. `[&]`：使用引用捕获运算符`&`来捕获类中的所有成员，以便在函数对象中使用。
4. `std::shared_ptr<PatternRoutingNode> current`：定义一个指向PatternRoutingNode的shared_ptr，用于存储当前遍历到的节点。
5. `std::shared_ptr<SteinerTreeNode> steiner`：定义一个指向SteinerTreeNode的shared_ptr，用于存储当前遍历到的Steiner树节点。
6. `constructDag(std::shared_ptr<PatternRoutingNode>&, std::shared_ptr<SteinerTreeNode>&)`：函数对象中的一个成员函数，用于递归地遍历Steiner树。
7. `numDagNodes++`：在每次调用`constructDag`函数时，自增全局变量`numDagNodes`，用于统计DAG中的节点数。
8. `constructPaths(dstNode, current)`：一个成员函数，用于构造从当前节点到目标节点的路径。
9. `if (dstNode == nullptr)`：判断`dstNode`是否为空，如果为空，则将`current`节点赋给`dstNode`。
10. `else {`：否则，将`current`节点添加到`dstNode`的子节点列表中，并调用`constructPaths`函数。
11. `constructDag(routingDag, steinerTree);`：调用函数对象，传入`routingDag`和`steinerTree`作为参数。
*/
...
void PatternRoute::constructRoutingDAGfixed(std::vector<int>& ChoosedPatternIndexArray,int& TwoPinNetInedex) {
    // 定义一个函数，用于构建路由DAG
    //std::ofstream outf;    //用ofstream类定义输入对象
    //outf.open("a.txt", std::ios::app);
    //outf.open("a.txt");
...
// 为PatternRoute类添加构造路径函数，参数为PatternRoutingNode类型的指针和int类型的childIndex
...
void PatternRoute::constructPathsfixed(std::shared_ptr<PatternRoutingNode>& start, std::shared_ptr<PatternRoutingNode>& end,std::vector<int>& ChoosedPatternIndexArray,int& TwoPinNetInedex,int childIndex) {
    // 如果childIndex为-1，则将其设置为start->paths的最后一个元素的索引
    if (childIndex == -1) {
        childIndex = start->paths.size();
        start->paths.emplace_back();
    ...
    if (start->x == end->x || start->y == end->y) {
        ...
        // 计算中间点
        utils::PointT<int> midPoint = ChoosedPatternIndexArray[TwoPinNetInedex] ? utils::PointT<int>(start->x, end->y) : utils::PointT<int>(end->x, start->y);
        // 创建一个PatternRoutingNode类型的指针，并设置其x和y坐标为中间点，numDagNodes自增，paths为一个元素，即end
        std::shared_ptr<PatternRoutingNode> mid = std::make_shared<PatternRoutingNode>(midPoint, numDagNodes++, true);
        mid->paths = {{end}};
        // 将mid添加到childPaths中

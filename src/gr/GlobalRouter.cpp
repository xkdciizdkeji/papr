...
void GlobalRouter::route()
{
    ...
    int a=1;
    // log() << "befor mazerouting NO." << a << " time" << std::endl;
    // for(int l=0;l<10;l++){
    //     std::cout<<"layer"<< l <<":"<<std::endl;
    //     for(int y=0;y<1144;y++){
    //         for(int x=0;x<844;x++){
    //             std::cout<<"("<<l<<","<<y<<","<<x<<")"<<":"<<gridGraph.getEdge(l,x,y).demand<<"|"<<gridGraph.getEdge(l,x,y).capacity;
    //             std::cout<< " ";
    ...
    auto t_outf_all_pin_location_begin = std::chrono::high_resolution_clock::now();
    std::ofstream outf_all_pin_location,outf_argv,outfb;    //这两个写文件的不需要时可以删掉
    
    outf_all_pin_location.open("all_pin_location_array.txt");
    outf_all_pin_location<<"[";
    outf_argv.open("argv.txt");
    outf_argv<<"["<<gridGraph.getSize(0)<<","<<gridGraph.getSize(1)<<"]";
    outf_argv.close();
    outfb.open("b.txt");
    outfb<<"[";
    ...
    for (const int netIndex : netIndices) {
        ...
        PatternRoutes.insert(std::make_pair(netIndex,patternRoute));//似乎是标记了相关关系
    ...
    for (const vector<int>& batch : batches) {
        runJobsMT(batch.size(),numofThreads, [&](int jobIdx) {
            ...
            outf_all_pin_location<< patternRoute.getsT()->getPythonStringfixed(patternRoute.getsT())<<std::endl;
        });
    }
    outf_all_pin_location<<"]";
    outf_all_pin_location.close();
    double t_outf_all_pin_location = std::chrono::duration<double>(std::chrono::high_resolution_clock::now() - t_outf_all_pin_location_begin).count();
    log() << "stage 1: pattern routing:cpp_write_txt_time: "<<t_outf_all_pin_location << std::endl;


    system("cd ..");
    system("python ../papr_v0.1.py");
    double t_python_script_runtime = std::chrono::duration<double>(std::chrono::high_resolution_clock::now() - t_outf_all_pin_location_begin).count()-t_outf_all_pin_location;
    log() << "stage 1: pattern routing:call_python_script_total_time: "<<t_python_script_runtime << std::endl;
    // Py_Initialize();
    // // 2.1 检查初始化是否成功
    // if (!Py_IsInitialized()) {
    ...
    // FILE* fp = fopen("../papr_v0.0.py", "r");
    // PyRun_AnyFile(fp, "../papr_v0.0.py");

    // PyObject* func = PyObject_GetAttrString(mod, "main");
    
    // PyObject* tuple = PyTuple_New(3);
    // PyTuple_SetItem(tuple, 0, PyLong_FromLong(1000));
    // PyTuple_SetItem(tuple, 1, PyLong_FromLong(1000));
    // PyTuple_SetItem(tuple, 2, PyUnicode_FromString("./choosed_pattern_index_array.txt"));

    // //std::cout << PyLong_AsLong(PyObject_CallObject(func, tuple)) << std::endl;
    ...
    // void Py_Finalize();
    std::ifstream in("./choosed_pattern_index_array.txt");
    vector<int> choosed_pattern_index_array;
    // extern int choosed_pattern;extern vector<int> choosed_pattern_index_array;
    int choosed_pattern;
    while (in>>choosed_pattern)
    {
        choosed_pattern_index_array.push_back(choosed_pattern);
    }
    double t_read_choosed_pattern_index_array = std::chrono::duration<double>(std::chrono::high_resolution_clock::now() - t_outf_all_pin_location_begin).count()-t_outf_all_pin_location - t_python_script_runtime;
    log() << "stage 1: pattern routing:cpp_read_txt_time: "<<t_read_choosed_pattern_index_array << std::endl;
    int two_pin_net_index=0;
    //std::cout<<choosed_pattern_index_array<<std::endl;
    ...
    for (const vector<int>& batch : batches) {
        runJobsMT(batch.size(),numofThreads, [&](int jobIdx) {
            auto patternRoute = PatternRoutes.find(netIndices[batch[jobIdx]])->second;
            //outfa<< patternRoute.getsT()->getPythonString(patternRoute.getsT())<<std::endl;
            ...
            // outfb<< patternRoute.getrT()->getPythonStringfixed(patternRoute.getrT())<<std::endl;
    ...
    {
        //在这里加输出点的函数
        //gridGraph.outputTreeTrace(net.getRoutingTree());
    ...
    if (netIndices.size() > 0)
    {
        ...
        batches = getBatches(routers, netIndices, numofThreads);

...
void GlobalRouter::getGuides(const GRNet &net, vector<std::pair<std::pair<int, int>, utils::BoxT<int>>> &guides)
{
    ...
    //             // }
    //             // for (int layerIdx = gpt.layerIdx; layerIdx <= parameters.min_routing_layer + 1; layerIdx++) {
    //             int layerIdx = gpt.layerIdx;
    //             guides.emplace_back(std::make_pair(layerIdx, parameters.min_routing_layer + 1), utils::BoxT<int>(
    //                 max(gpt.x, 0),
    //                 max(gpt.y, 0),
    //                 min(gpt.x, (int)gridGraph.getSize(0) - 1),
    //                 min(gpt.y, (int)gridGraph.getSize(1) - 1)
    //             ));
    //             areaOfPinPatches += (guides.back().second.x.range() + 1) * (guides.back().second.y.range() + 1);
    //             // }
    //         }
    ...
    //                 GRPoint point = (direction == MetalLayer::H ? GRPoint(node->layerIdx, c, r) : GRPoint(node->layerIdx, r, c));
    //                 if (getSpareResource(point) < wire_patch_threshold) {
    //                     for (int layerIndex = node->layerIdx - 1; layerIndex < node->layerIdx; layerIndex += 2) {
    //                         if (layerIndex < parameters.min_routing_layer || layerIndex >= gridGraph.getNumLayers()) continue;
    //                         if (getSpareResource({layerIndex, point.x, point.y}) >= 1.0) {
    //                             guides.emplace_back(std::make_pair(layerIndex, layerIndex+2), utils::BoxT<int>(point.x, point.y));
    //                             areaOfWirePatches += 1;
    //                             patched = true;
    //                         }
    //                     }
...
void GlobalRouter::printStatistics() const
{
        ...
        GRTreeNode::preorder(net.getRoutingTree(), [&](std::shared_ptr<GRTreeNode> node)
            ...
            for (const auto& child : node->children) {
                if (node->layerIdx == child->layerIdx) {
                    ...
                    nonstack_via_counter[node->layerIdx][node->x][node->y] += 1;
                    nonstack_via_counter[child->layerIdx][child->x][child->y] += 1;
    ...
    // // Print wire usage
    // for (int l = 0; l < gridGraph.getNumLayers(); l++)
    // {
    //     log() << "Wire Usage - Layer: " << l << std::endl;
    //     for (int x = 0; x < gridGraph.getSize(0); x++)
    //     {
    //         for (int y = 0; y < gridGraph.getSize(1); y++)
    //         {
    //             std::cout << wireUsage[l][x][y] << " ";
    //         }
    //         std::cout << std::endl;
    //     }
    // }

    // // Print nonstack via count
    // for (int l = 0; l < gridGraph.getNumLayers(); l++)
    // {
    //     log() << "Nonstack Via Count - Layer: " << l << std::endl;
    //     for (int x = 0; x < gridGraph.getSize(0); x++)
    //     {
    //         for (int y = 0; y < gridGraph.getSize(1); y++)
    //         {
    //             std::cout << nonstack_via_counter[l][x][y] << " ";
    //         }
    //         std::cout << std::endl;
    //     }
    // }
    ...
    for (unsigned z = parameters.min_routing_layer; z < gridGraph.getNumLayers(); z++)
    {
        ...
        for (unsigned x = 0; x < gridGraph.getSize(0); x++)
        {
            for (unsigned y = 0; y < gridGraph.getSize(1); y++)
            {
                ...
                if (capacity > 0)
                {
                    ...
                    // printf("overflow = %lf\n", overflow);
                    // log()<<"overflow = "<<overflow<<std::endl;
        ...
        //             for (int y = 0; y < gridGraph.getSize(1); y++)
        //             {
        //                 CapacityT usage = 2 * wireUsage[l][x][y] + nonstack_via_counter[l][x][y];
        //                 CapacityT capacity = max(gridGraph.getEdge(l, x, y).capacity, 0.0);
        //                 if (usage > 2 * capacity)
        //                 {
        //                     num_overflows += usage - 2 * capacity;
        //                 }
        //                 // if (usage > 0.0 && usage > capacity && capacity > 0.0) {
        //                 if (capacity > 0.0)
        //                 {
        //                     // overflow = overflow + unit_length_short_costs[l] * (usage - capacity) * gridGraph.getEdgeLength(direction, x);
        //                     layer_overflows = layer_overflows + exp((double(usage - 2 * capacity)) / 2 * overflow_slope);
        //                     // }else if(usage > 0.0 && usage > capacity && capacity == 0.0){
        //                 }
        //                 else if (capacity == 0.0 && usage > 0)
        //                 {
        //                     layer_overflows = layer_overflows + exp(1.5 * double(usage - 2 * capacity) * overflow_slope);
        //                 }
        //             }
        //         }
        //         log() << "layer " << l << " num overflow: " << num_overflows << " layer_overflows " << layer_overflows << std::endl;
        //     }
        //     else
        //     { // vertical
        //         for (int x = 0; x < gridGraph.getSize(0); x++)

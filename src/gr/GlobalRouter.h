...
public:
    ...
    // void create_masks_fixed(int Layer_Num,int Gcell_Num_X, int Gcell_Num_Y, std::vector<TorchEdge> Two_Pin_net_vector,torch::Tensor *Demand_Mask, c10::Device device);//针对四维的P矩阵设置的mask
    // void divide_net(std::vector<TorchEdge> Two_Pin_net_vector);
    ...
    void create_masks_fixed_agian_batch(int Layer_Num,int Gcell_Num_X, int Gcell_Num_Y, std::vector<TorchEdge> Two_Pin_net_vector,torch::Tensor *Demand_Mask,torch::Tensor *Wirelength_Mask,torch::Tensor *Viacount_Mask, c10::Device device);//针对四维的P矩阵设置的mask
    // void create_masks_fixed_agian_batch(int Layer_Num,int Gcell_Num_X, int Gcell_Num_Y, std::vector<TorchEdge> Two_Pin_net_vector,std::vector<torch::Tensor> *Demand_Mask,torch::Tensor *Wirelength_Mask,torch::Tensor *Viacount_Mask, c10::Device device,int batch_size);//针对四维的P矩阵设置的mask
...
struct Torchroute : torch::nn::Module
{
    ...
    torch::Tensor forwardfixed(int net_num,int max_tree_num_in_one_single_net,int All_Two_Pin_Net_Num, int pn, torch::Tensor *demand_mask,torch::Tensor *Wirelength_Mask,torch::Tensor *Viacount_Mask,torch::Tensor *P_tree_mask_to_P_pattern, torch::Tensor& capacity, int layersize,int xsize, int ysize, c10::Device dev)
	{
        ...
        // std::cout<<torch::sum(all_gcell_overflow)<<" "<<torch::sum(all_gcell_demand)<<" "<<via_cost<<std::endl;

        auto total_loss = torch::sum(all_gcell_overflow)*5000+torch::sum(all_gcell_demand)+via_cost*10;
        // auto total_loss = torch::sum(all_gcell_overflow)*5000+via_cost*10;
		return total_loss;
	}
    torch::Tensor forwardfixed_batch(int net_num,int max_tree_num_in_one_single_net,int All_Two_Pin_Net_Num, int pn, torch::Tensor *demand_mask,torch::Tensor *Wirelength_Mask,torch::Tensor *Viacount_Mask,torch::Tensor *P_tree_mask_to_P_pattern, torch::Tensor& capacity, int layersize,int xsize, int ysize, c10::Device dev,int batch_index,int batch_size,torch::Tensor& all_gcell_demand)
	{
        ...
        //auto P_tree_reshape =(torch::_sparse_mm(*P_tree_mask_to_P_pattern,P_tree.reshape({net_num*max_tree_num_in_one_single_net,1}))).reshape({All_Two_Pin_Net_Num,pn});
        //auto P_tree_reshape =(torch::_sparse_mm(*P_tree_mask_to_P_pattern,P_tree.reshape({net_num*max_tree_num_in_one_single_net,1}))).repeat({1,pn}).reshape({All_Two_Pin_Net_Num*pn, 1}).to(dev);//P_tree_reshape的尺寸为：{All_Two_Pin_Net_Num*pn, 1}
        // std::cout<<P_tree_mask_to_P_pattern->sizes()<<" "<<P_tree_reshpe_temp.sizes()<<std::endl;
        auto P_tree_reshape =(torch::_sparse_mm(*P_tree_mask_to_P_pattern,P_tree_reshpe_temp)).repeat({1,pn}).reshape({All_Two_Pin_Net_Num*pn, 1}).to(dev);//P_tree_reshape的尺寸为：{All_Two_Pin_Net_Num*pn, 1}
        auto p_reshape = P_pattern_reshape * P_tree_reshape;
        // auto p_reshape_need =p_reshape.index({torch::indexing::Slice(batch_index*batch_size*pn,(batch_index+1)*batch_size*pn)});//有问题
        // auto demand_mask_need =demand_mask.index({torch::indexing::Slice(), torch::indexing::Slice(0, 1000)});;
        // auto Wirelength_Mask_need =Wirelength_Mask[batch_index].index({torch::indexing::Slice(batch_index*batch_size,(batch_index+1)*batch_size)});
        //下面是提取demand_mask_need
        // 指定要提取的列范围
        // int64_t col_start = batch_index*batch_size;
        // int64_t col_end = (batch_index+1)*batch_size; 
        // // 从稀疏张量中提取 indices_tensor 和 values_tensor
        // torch::Tensor recovered_indices = demand_mask->indices();
        // torch::Tensor recovered_values = *demand_mask._values();
        all_gcell_demand=torch::mm(*demand_mask, p_reshape);
        ...
        auto total_loss = torch::sum(all_gcell_overflow)*5000+torch::sum(all_gcell_demand)+via_cost*8;
    ...
    //     auto total_loss = torch::sum(all_gcell_overflow)*5000+torch::sum(all_gcell_demand);
